    ...
    <nav className="fixed top-0 w-full z-50 glass-nav shadow-sm">
      <div className="flex justify-between items-center px-4 md:px-8 h-20 max-w-full mx-auto">
        <div className="flex items-center gap-4 md:gap-12">
          <Link href="/" className="text-xl md:text-2xl font-black tracking-tighter text-blue-600 font-headline">
            EdTech Canvas
          </Link>
          <div className="hidden md:flex items-center gap-8 font-headline tracking-tight font-semibold text-sm">
            ...
            <Link href="/blog" className="text-slate-600 hover:text-blue-600 transition-all duration-300">
              Blog
            </Link>
            <Link href="/courses" className="text-slate-600 hover:text-blue-600 transition-all duration-300">
              Pathways
            </Link>
            <Link href="/about" className="text-slate-600 hover:text-blue-600 transition-all duration-300">
              About
            </Link>
          </div>
        </div>
      ...
      {isMenuOpen && (
        <div className="md:hidden absolute top-20 left-0 w-full bg-white border-b border-surface-container shadow-lg py-4 px-4 flex flex-col gap-4">
          ...
          <Link href="/courses" className="text-slate-600 font-semibold py-2" onClick={() => setIsMenuOpen(false)}>Courses</Link>
          <Link href="/blog" className="text-slate-600 font-semibold py-2" onClick={() => setIsMenuOpen(false)}>Blog</Link>
